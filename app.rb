...
class GithubSearchApp < Roda
  ...
  plugin :route_csrf
  plugin :flash
  plugin :assets, css: 'app.css', css_opts: css_opts, timestamp_paths: true
  plugin :render, escape: true, layout: './layout',
                  template_opts: { chain_appends: !defined?(SimpleCov), freeze: true,
  ...
  plugin :error_handler do |e|
    ...
    when Roda::RodaPlugins::RouteCsrf::InvalidToken
      ...
      response.status = 400
      view(content: '<p>An invalid security token was submitted with this request,' \
                    ' and this request could not be processed.</p>')
    else
  ...
  plugin :sessions,
         key: '_GithubSearchApp.session',
         # cookie_options: {secure: ENV['RACK_ENV'] != 'test'}, # Uncomment if only allowing https:// access
         secret: ENV.send((ENV['RACK_ENV'] == 'development' ? :[] : :delete), 'GITHUB_SEARCH_APP_SESSION_SECRET')
